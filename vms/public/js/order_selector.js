...
vms.OrderSelector = class OrderSelector {
    ...
    setup_wrapper() {
			...
			<table class="table">
				...
				<tbody>
					...
					<tr>
						...
						<td class="pop_rem_qty text-center">${flt(this.frm.doc.qty_capacity - this.frm.doc.allocated_qty, 2)}</td>
						<td class="pop_rem_wt text-center">${flt(this.frm.doc.weight_capacity - this.frm.doc.allocated_weight, 2)}</td>
					</tr>
				</tbody>
			</table>
		`);
            ...
            if ($(e.target).is(":checked")) {
                ...
                unchecked_items.forEach((item) => {
                    if (this.allocated_order_items[item.sales_order]?.[item.sales_order_detail]) {
                        delete this.allocated_order_items[item.sales_order][item.sales_order_detail];
                    }
        ...
        this.results.on("click", ".list-row-check", (e) => {
            ...
            if ($(e.target).is(":checked") && !this.allocated_order_items[selected_so][so_detail]) {
        ...
        this.orders.on("click", ".list-row-check", (e) => {
            ...
            if (!selected_so) return;
            if ($(e.target).is(":checked")) {
                this.orders.find(".list-item-container .list-row-check").filter((i, el) => {
            ...
            if (!items) return;
            items.forEach((item) => {
                item.checked = this.allocated_order_items[selected_so]?.[item.sales_order_detail] ? true : false;
    ...
    set_selected_order_bg(order, colour) {
        this.orders.find(".list-item-container").filter((i, el) => {
    ...
    refresh_capacity() {
        ...
        if (this.frm.doc.qty_capacity > allocated_qty) {
            $(".pop_rem_qty").html(`<span style="color: green">${flt(this.frm.doc.qty_capacity - allocated_qty, 2)}</span>`);
        } else {
            $(".pop_rem_qty").html(`<span style="color: red">${flt(this.frm.doc.qty_capacity - allocated_qty, 2)}</span>`);
        }

        if (this.frm.doc.weight_capacity > allocated_wt) {
            $(".pop_rem_wt").html(`<span style="color: green">${flt(this.frm.doc.weight_capacity - allocated_wt, 2)}</span>`);
        } else {
            $(".pop_rem_wt").html(`<span style="color: red">${flt(this.frm.doc.weight_capacity - allocated_wt, 2)}</span>`);
        }
            ...
            <div class="list-item">
		        <div class="list-item__content" style="flex: 0 0 10px;">
			        <input
                        ...
                        ${result.sales_order_detail ? `data-so-detail="${result.sales_order_detail}"` : ""}
        ...
        return $results
            ...
            .map(function () {
                ...
                if ($(this).find(".list-row-check:checkbox:checked").length > 0) {
                    ...
                    checked_values["transaction_date"] = $(this).attr(
